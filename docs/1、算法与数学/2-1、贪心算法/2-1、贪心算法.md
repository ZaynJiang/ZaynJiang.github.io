...
![image-20220617222739411](image-20220617222739411.png) 
...
public boolean carPooling(int[][] trips, int capacity) {
    ...
    for (int i = 0; i < path.length && capacity >= 0; i++) {
        capacity -= path[i];
    }
    return capacity >= 0;
}
...
![image-20220628111036781](image-20220628111036781.png) 
...
public boolean canPlaceFlowers(int[] flowerbed, int n) {
    int step = 0, len = flowerbed.length;
    while (step < len && n >= 0) {
        if (flowerbed[step] == 1) {
            step+=2;
        } else if (step == len - 1 || flowerbed[step + 1] == 0) {
            n--;
            step += 2;
        } else {
            step += 3;
