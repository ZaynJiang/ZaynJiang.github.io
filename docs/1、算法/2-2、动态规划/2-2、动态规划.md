...
* 【518】给你一个整数数组 coins 表示不同面额的硬币，另给一个整数 amount 表示总金额。请你计算并返回可以凑成总金额的硬币组合数。如果任何硬币组合都无法凑出总金额，返回 0 
...
![image-20220612145239437](image-20220612145239437.png) 
...
```
    public int maxSumTwoNoOverlap(int[] nums, int m, int n) {
        for (int i = 1; i < nums.length; i++) {
            nums[i] += nums[i - 1];
        }
        int mMax = nums[m - 1], nMax = nums[n - 1], ans = nums[m + n - 1];
        for (int i = m + n; i < nums.length; i++) {
            // 分别以L在前M在后，M在前L在后两种方式，滑动窗口，每次移动一格。
            mMax = Math.max(mMax, nums[i - n] - nums[i - m - n]);
            nMax = Math.max(nMax, nums[i - m] - nums[i - m - n]);
            ans = Math.max(ans, Math.max(mMax + nums[i] - nums[i - n], nMax + nums[i] - nums[i - m]));
        }
        return ans;
    }
```
...
def maxSumTwoNoOverlap(A, L, M):
    ...
    for i in range(L+M, n):
        # 当L在M前时，i代表M的最后一个索引,此时M已确定，分别以L在前M在后，M在前L在后两种方式，滑动窗口，每次移动一格。
...
public int waysToChange(int n) {
    int[] dp = new int[n + 1], coins = new int[]{25, 10, 5, 1};
    dp[0] = 1;
    for (int coin : coins) {
...
我们可以直接模拟整个过程。我们记录流入每个杯子的香槟的数量之和，例如对于一个杯子，流入的香槟数量为10。由于这个数值大于1，因此会有9数量的香槟流出这个杯子，并且会有4.5数量的香槟分别流入这个杯子下面的两个杯子中。以此类推。
总的来说，如果流入一个杯子的香槟数量为X，且X大于1，那么就会有Q= (X- 1.0)/2数量的香槟流入下面的两个杯子中。我们可以从第一层的杯子开始，计算出第二层每个杯子中流入的香槟数量，再计算出第三层的数量，直到计算到第query_row层。
...
```
    public double champagneTower(int poured, int query_row, int query_glass) {
        ...
        return Math.min(1.00, dp[query_row][query_glass]);
    }
```
...
public int mincostTickets(int[] days, int[] costs) {
    ...
    for (int i = 1; i < dp.length; i++) {
        ...
        dp[i] = Math.min(Math.min(n2, n3), n1);
    }
...
![image-20220622204656547](image-20220622204656547.png) 
...
![image-20220623190930583](image-20220623190930583.png) 
...
public int longestSubsequence(int[] arr, int difference) {
    ...
    int ans = 0;
    for (int i : arr) {
        hash.put(i, hash.getOrDefault(i - difference, 0) + 1);
        ans = Math.max(hash.get(i), ans);
    }
    return ans;
}
```
...
```
动态规划
和给你一个字符串 s 和一个字符串列表 wordDict 作为字典。请你判断是否可以利用字典中出现的单词拼接出 s 。这个类似
注意不能遍历set，这样会使得时间复杂度上升，可以遍历单词，看符不符合在dict之中
public List<String> findAllConcatenatedWordsInADict(String[] words) {
    Set<String> dict = new HashSet<>();
    List<String> ans = new ArrayList<>();
    Arrays.stream(words).sorted((o1, o2) -> o1.length() - o2.length()).forEach(w -> {
        if (w.length() > 0 && canFrom(w, dict)) {
            ans.add(w);
        }
        dict.add(w);
    });
...
public boolean canFrom(String word, Set<String> dict) {
    if (word.isEmpty()) {
        return false;
    }
...
![image-20220626162127558](image-20220626162127558.png)

![image-20220626170314790](image-20220626170314790.png) 
...
class Solution {
    ...
    public int mergeStones(int[] stones, int k) {
        ...
        int MAX = 99999999; // 不用Integer.MAX_VALUE,因为Integer.MAX_VALUE + 正数 会溢出变为负数
        for (int i = 1; i <= n; i++) {
            for (int j = i; j <= n; j++) {
                for (int m = 2; m <= k; m++) {
                    dp[i][j][m] = MAX;//初始化
            ...
            dp[i][i][1] = 0;//自己合成1堆的成本为0
        }

        for (int len = 2; len <= n; len++) {//遍历区间长度
            for (int i = 1; i + len - 1 <= n; i++) {//遍历起始下标
                int j = i + len - 1;
                for (int m = 2; m <= k; m++) {//遍历2以上堆数的成本
                    for (int p = i; p < j; p += (k - 1)) {//遍历分界点
                        dp[i][j][m] = Math.min(dp[i][j][m], dp[i][p][1] + dp[p + 1][j][m-1]);//选取分割最小值
                    }
                }
                dp[i][j][1] = pre[j] - pre[i - 1] + dp[i][j][k];//合成1堆等于合成k堆的成本家伙加上它们的前缀和
            }
        }
        return dp[1][n][1];
    }
...
![image-20220629204351746](image-20220629204351746.png) 
